...
const SendMoney = () => {
 ...
 (u) => u.role === "user" && u.status === "approved" && u.email !== user?.email
 );
 ...
 try {
 const { data } = await axiosSecure.post("/sendMoney", {
 recipientEmail,
 amount,
 pin,
 });
 ...
 if (isSuccess) {
 return <TransactionSuccess amount={amount} recipient={recipientEmail} type="send-money" />;
 }
 ...
 <input
 className="w-full px-4 py-3.5 rounded-xl border border-base-300 focus:outline-none focus:border-[#1A3626] focus:ring-1 focus:ring-[#1A3626] transition-colors text-sm text-base-content bg-base-200 tracking-[0.3em] font-bold"
 type="password"
 placeholder="••••"
 value={pin}
