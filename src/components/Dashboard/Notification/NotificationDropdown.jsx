  ...
  return (
    <div className="relative">
      ...
      <button 
        ...
        className="relative p-2 bg-white rounded-full text-gray-500 hover:text-[#1A3626] transition-colors shadow-sm border border-gray-100 focus:outline-none"
      >
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 top-full mt-3 w-80 md:w-96 bg-white rounded-2xl shadow-xl border border-gray-100 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
            {/* Header */}
            <div className="p-4 border-b border-gray-50 flex justify-between items-center bg-gray-50/50">
              ...
              <div className="flex items-center gap-3">
                <button 
                  ...
                  className="text-gray-400 hover:text-[#1A3626] transition-colors"
                  title="Refresh"
                >
                ...
                {unreadCount > 0 && (
                  <button 
                    ...
                    className="text-[11px] font-bold text-[#1A3626] hover:underline"
                  >
                    Mark all as read
                  </button>
                )}
            ...
            <div className="max-h-[400px] overflow-y-auto">
              {notifications && notifications.length > 0 ? (
                notifications.map((notif) => (
                  <div 
                    key={notif._id} 
                    className={`p-4 flex gap-4 border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors cursor-pointer ${!notif.isRead ? 'bg-blue-50/30' : ''}`}
                  >
                    ...
                    {!notif.isRead && (
                      <div className="w-2 h-2 bg-[#1A3626] rounded-full mt-2"></div>
                    )}
                  </div>
                ))
              ) : (
            ...
            <div className="p-3 border-t border-gray-50 text-center bg-gray-50/50">
               <button className="text-xs font-bold text-gray-500 hover:text-[#1A3626] transition-colors">
